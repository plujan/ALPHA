...
// numbers from AN 2010 371
double sigmaPtJet(const double& pt, const double& eta) {
  double N,S,C,m;
  if (fabs(eta)<0.5) {
    N=3.969;
    S=0.183;
  ...
  return sqrt(N*N/(pt*pt)+S*S*pow(pt,(m-1.0))+C*C);
}
...
void testKinFitFWLite()
{
  ...
  m1(0,0) = pow(sigmaPtJet(v1.Et(), v1.Eta())*v1.Et(),2);
  m1(1,1) = ErrEta(v1.Et(), v1.Eta()); // eta
  m1(2,2) = ErrPhi(v1.Et(), v1.Eta()); // phi
  cout << "ErrEt " << sqrt(m1(0,0)) << endl;
  cout << "SigmaPt " << sigmaPtJet(v1.Et(), v1.Eta())*v1.Et() << endl;
  cout << "ErrEta " << sqrt(m1(1,1)) << endl;
  cout << "ErrPhi " << sqrt(m1(2,2)) << endl;
  //m2(0,0) = ErrEt (v2.Et(), v2.Eta()); // et
  m2(0,0) = pow(sigmaPtJet(v2.Et(), v2.Eta())*v2.Et(),2);
  ...
  print(fitter);
  cout << "Jet1 " << jet1->getCurr4Vec()->Pt() << " vs " << jet1->getIni4Vec()->Pt() << " " <<sqrt(m1(0,0)) << endl;
  cout << "Jet2 " << jet2->getCurr4Vec()->Pt() << " vs " << jet2->getIni4Vec()->Pt() << " " <<sqrt(m2(0,0)) << endl;
  cout << "Jet3 " << jet3->getCurr4Vec()->Pt() << " vs " << jet3->getIni4Vec()->Pt() << " " <<sqrt(m3(0,0)) << endl;

  double chi2=pow(jet1->getCurr4Vec()->Pt()-jet1->getIni4Vec()->Pt(),2)/m1(0,0);
  chi2+=pow(jet1->getCurr4Vec()->Eta()-jet1->getIni4Vec()->Eta(),2)/m1(1,1);
  chi2+=pow(jet1->getCurr4Vec()->Phi()-jet1->getIni4Vec()->Phi(),2)/m1(2,2);

  chi2+=pow(jet2->getCurr4Vec()->Pt()-jet2->getIni4Vec()->Pt(),2)/m2(0,0);
  chi2+=pow(jet2->getCurr4Vec()->Eta()-jet2->getIni4Vec()->Eta(),2)/m2(1,1);
  chi2+=pow(jet2->getCurr4Vec()->Phi()-jet2->getIni4Vec()->Phi(),2)/m2(2,2);

  chi2+=pow(jet3->getCurr4Vec()->Pt()-jet3->getIni4Vec()->Pt(),2)/m3(0,0);
  chi2+=pow(jet3->getCurr4Vec()->Eta()-jet3->getIni4Vec()->Eta(),2)/m3(1,1);
  chi2+=pow(jet3->getCurr4Vec()->Phi()-jet3->getIni4Vec()->Phi(),2)/m3(2,2);
  cout << "chi2 " << chi2 << " NDoF " << fitter->getNDF() << 
    " " << TMath::Prob(chi2,fitter->getNDF()) << 
    " " << TMath::Prob(chi2,7) << endl;
