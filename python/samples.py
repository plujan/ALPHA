#! /usr/bin/env python

#voms-proxy-init -voms cms
#

sample = {
    ########## DATA ##########
    ##ReMiniAOD##
    'METRun2016B-03Feb2017_ver1-v1' : {
        'nevents' : 583427,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016B-03Feb2017_ver2-v2' : {
        'nevents' : 35987712,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016C-03Feb2017-v1' : {
        'nevents' : 17381222,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016D-03Feb2017-v1' : {
        'nevents' : 20947429,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016E-03Feb2017-v1' : {
        'nevents' : 22348402,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016F-03Feb2017-v1' : {
        'nevents' : 13319829,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016G-03Feb2017-v1' : {
        'nevents' : 26974131,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016H-03Feb2017_ver2-v1' : {
        'nevents' : 38957320,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'METRun2016H-03Feb2017_ver3-v1' : {
        'nevents' : 816165,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },    
    ##ReMiniAOD##
    'JetHTRun2016B-03Feb2017_ver1-v1' : {
        'nevents' : 583427,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'JetHTRun2016B-03Feb2017_ver2-v2' : {
        'nevents' : 35987712,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'JetHTRun2016C-03Feb2017-v1' : {
        'nevents' : 17381222,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'JetHTRun2016D-03Feb2017-v1' : {
        'nevents' : 20947429,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'JetHTRun2016E-03Feb2017-v1' : {
        'nevents' : 22348402,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'JetHTRun2016F-03Feb2017-v1' : {
        'nevents' : 13319829,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'JetHTRun2016G-03Feb2017-v1' : {
        'nevents' : 26974131,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'JetHTRun2016H-03Feb2017_ver2-v1' : {
        'nevents' : 38957320,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'JetHTRun2016H-03Feb2017_ver3-v1' : {
        'nevents' : 816165,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },    
    ########## DATA ##########
    'METRun2016B-23Sep2016-v2' : {
        'nevents' : 583427,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016B-23Sep2016-v3' : {
        'nevents' : 35987712,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016C-23Sep2016-v1' : {
        'nevents' : 17381222,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016D-23Sep2016-v1' : {
        'nevents' : 20947429,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016E-23Sep2016-v1' : {
        'nevents' : 22348402,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016F-23Sep2016-v1' : {
        'nevents' : 13319829,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016G-23Sep2016-v1' : {
        'nevents' : 26974131,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
        },
    'METRun2016H-PromptReco-v1' : {
        'nevents' : 13873,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'METRun2016H-PromptReco-v2' : {
        'nevents' : 39496995,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'METRun2016H-PromptReco-v3' : {
        'nevents' : 1129511,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },    

    ### SingleMu
    'SingleMuonRun2016B-23Sep2016-v1' : {
        'nevents' : 2789243,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
   'SingleMuonRun2016B-23Sep2016-v3' : {
        'nevents' : 158145722,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleMuonRun2016C-23Sep2016-v1' : {
        'nevents' : 67441308,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleMuonRun2016D-23Sep2016-v1' : {
        'nevents' : 98017996,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleMuonRun2016E-23Sep2016-v1' : {
        'nevents' : 90984718,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleMuonRun2016F-23Sep2016-v1' : {
        'nevents' : 65425108,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleMuonRun2016G-23Sep2016-v1' : {
        'nevents' : 149916849,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleMuonRun2016H-PromptReco-v1' : {
        'nevents' : 29162,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleMuonRun2016H-PromptReco-v2' : {
        'nevents' : 171134793,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleMuonRun2016H-PromptReco-v3' : {
        'nevents' : 4393222,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    ### SingleEle
    'SingleElectronRun2016B-23Sep2016-v2' : {
        'nevents' : 1422819,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleElectronRun2016B-23Sep2016-v3' : {
        'nevents' : 246440440,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleElectronRun2016C-23Sep2016-v1' : {
        'nevents' : 97259854,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleElectronRun2016D-23Sep2016-v1' : {
        'nevents' : 148167727,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleElectronRun2016E-23Sep2016-v1' : {
        'nevents' : 117321545,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleElectronRun2016F-23Sep2016-v1' : {
        'nevents' : 70593532,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleElectronRun2016G-23Sep2016-v1' : {
        'nevents' : 153363109,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleElectronRun2016H-PromptReco-v2' : {
        'nevents' : 126863489,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'SingleElectronRun2016H-PromptReco-v3' : {
        'nevents' : 3191585,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    'BTagCSVRun2016B-23Sep2016-v2' : {
        'nevents' : 1972666,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BTagCSVRun2016B-23Sep2016-v3' : {
        'nevents' : 77890616,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BTagCSVRun2016C-23Sep2016-v1' : {
        'nevents' : 30358567,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BTagCSVRun2016D-23Sep2016-v1' : {
        'nevents' : 56527008,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BTagCSVRun2016E-23Sep2016-v1' : {
        'nevents' : 60415444,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BTagCSVRun2016F-23Sep2016-v1' : {
        'nevents' : 37608672,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BTagCSVRun2016G-23Sep2016-v1' : {
        'nevents' : 100834056,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BTagCSVRun2016H-PromptReco-v2' : {
        'nevents' : 64179785,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BTagCSVRun2016H-PromptReco-v3' : {
        'nevents' : 1695733,
        'xsec'    : 1.,
        'matcheff': 1.,
        'kfactor' : 1.,  
    },

    ########## MC ##########
    #DYJetsToNuNu_PtZ
    'DYJetsToNuNu_PtZ-50To100_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 21953584,
        'xsec'    : 593.9,#taken from mcm
        'matcheff': 0.349,
        'kfactor' : 1.,
    },
    'DYJetsToNuNu_PtZ-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 5353639,
        'xsec'    : 170.4,#Check!!! Totally different on mcm!!!!
        'matcheff': 0.38,
        'kfactor' : 1.,
    },
    'DYJetsToNuNu_PtZ-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 5356674,
        'xsec'    : 170.4,
        'matcheff': 0.38,
        'kfactor' : 1.,
    },
    'DYJetsToNuNu_PtZ-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 1059634,
        'xsec'    : 6.636, #2.082,#
        'matcheff': 0.4,
        'kfactor' : 1.,
    },
    'DYJetsToNuNu_PtZ-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 1050705,
        'xsec'    : 0.9372, #0.2816,#
        'matcheff': 0.41,
        'kfactor' : 1.,
    },
    'DYJetsToNuNu_PtZ-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 1050592,
        'xsec'    : 0.9372, #0.2816,#
        'matcheff': 0.41,
        'kfactor' : 1.,
    },
    'DYJetsToNuNu_PtZ-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 1022595,
        'xsec'    : 0.1042, #0.02639,#
        'matcheff': 0.42,
        'kfactor' : 1.,
    },
    'DYJetsToNuNu_PtZ-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 1024620,
        'xsec'    : 0.1042, #0.02639,#
        'matcheff': 0.42,
        'kfactor' : 1.,
    },

    #ZJetsToNuNu_HT
    'ZJetsToNuNu_HT-100To200_13TeV-madgraph-v1' : {
        'nevents' : 5246318,
        'xsec'    : 280.5,# <--monojet ##280.47,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.626,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-100To200_13TeV-madgraph_ext1-v1' : {
        'nevents' : 19026540,
        'xsec'    : 280.5,# <--monojet ##280.47,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.626,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-200To400_13TeV-madgraph-v1' : {
        'nevents' : 5132650,
        'xsec'    : 77.7,# <--monojet ##78.36,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.617,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-200To400_13TeV-madgraph_ext1-v1' : {
        'nevents' : 19612266,
        'xsec'    : 77.7,# <--monojet ##78.36,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.617,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-400To600_13TeV-madgraph-v1' : {
        'nevents' : 1020309,
        'xsec'    : 10.71,# <--monojet ##10.944,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.459,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-400To600_13TeV-madgraph_ext1-v1' : {
        'nevents' : 8842560,
        'xsec'    : 10.71,# <--monojet ##10.944,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.459,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-600To800_13TeV-madgraph-v1' : {
        'nevents' : 5763506,
        'xsec'    : 2.562,# <--monojet ##4.203,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.391,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-800To1200_13TeV-madgraph-v1' : {
        'nevents' : 2170137,
        'xsec'    : 1.183,# <--monojet ##2.109,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.391,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-1200To2500_13TeV-madgraph-v1' : {
        'nevents' : 369514,
        'xsec'    : 0.286,# <--monojet ##0.1795,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.391,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-1200To2500_13TeV-madgraph_ext1-v1' : {
        'nevents' : 143957,
        'xsec'    : 0.286,# <--monojet ##0.1795,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.391,# <--Alberto ## 
    },
    'ZJetsToNuNu_HT-2500ToInf_13TeV-madgraph-v1' : {
        'nevents' : 405030,
        'xsec'    : 6.976e-04,# <--monojet ##0.004011,# <--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.23,# <--flat ##1.391,# <--Alberto ## 
    },


    #WJetsToLNu_HT*
#    'WJetsToLNu_HT-100To200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
#        'nevents' : 10235198,
#        'xsec'    : 1343.,#<--Monojet ## 13072.8173/10.,#<-- Julie H ## 1292.,#<--Alberto ##
#        'matcheff': 1.,
#        'kfactor' : 1./0.998056,#<-- Julie H ## 1.21,#<--Flat ## 1.459,#<--Alberto ##
#    },
   'WJetsToLNu_HT-100To200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 29503700,
        'xsec'    : 1343.,#<--Monojet ## 13072.8173/10.,#<-- Julie H ## 1292.,#<--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.395,#<--Lisav1 ## 1.400,#<--Lisav2 ## 1.21,#<--Flat ## 1./0.998056,#<-- Julie H ## 1.459,#<--Alberto ##  
    },
#    'WJetsToLNu_HT-100To200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext2-v1' : {
#        'nevents' : 39617787,
#        'xsec'    : 1343.,#<--Monojet ## 13072.8173/10.,#<-- Julie H ## 1292.,#<--Alberto ##
#        'matcheff': 1.,
#        'kfactor' : 1./0.998056,#<-- Julie H ## 1.21,#<--Flat ## 1.459,#<--Alberto ##
#    },
#    'WJetsToLNu_HT-200To400_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
#        'nevents' : 4950373,
#        'xsec'    : 359.6,#<--Monojet ## 4223.9214/10.,#<-- Julie H ## 385.9,#<--Alberto ##
#        'matcheff': 1.,
#        'kfactor' : 1./0.978569,#<-- Julie H ## 1.21,#<--Flat ## 1.434,#<--Alberto ##
#    },
    'WJetsToLNu_HT-200To400_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 14815928,
        'xsec'    : 359.6,#<--Monojet ## 4223.9214/10.,#<-- Julie H ## 385.9,#<--Alberto #### 
        'matcheff': 1.,
        'kfactor' : 1.526,#<--Lisav1 ## 1.531,#<--Lisav2 ## 1.21,#<--Flat ## 1./0.978569,#<-- Julie H ## 1.434,#<--Alberto ##  
    },
#    'WJetsToLNu_HT-200To400_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext2-v1' : {
#        'nevents' : 19914590,
#        'xsec'    : 359.6,#<--Monojet ## 4223.9214/10.,#<-- Julie H ## 385.9,#<--Alberto ##
#        'matcheff': 1.,
#        'kfactor' : 1./0.978569,#<-- Julie H ## 1.21,#<--Flat ## 1.434,#<--Alberto ##
#    },
#    'WJetsToLNu_HT-400To600_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
#        'nevents' : 1963464,
#        'xsec'    : 48.85,#<--Monojet ## 627.9155/10.,#<-- Julie H ## 47.9,#<--Alberto ##
#        'matcheff': 1.,
#        'kfactor' : 1./0.928054,#<-- Julie H ## 1.21,#<--Flat ## 1.532,#<--Alberto ##
#    },
    'WJetsToLNu_HT-400To600_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 5796237,
        'xsec'    : 48.85,#<--Monojet ## 627.9155/10.,#<-- Julie H ## 47.9,#<--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.679,#<--Lisav1 ## 1.715,#<--Lisav2 ## 1.21,#<--Flat ## 1./0.928054,#<-- Julie H ## 1.532,#<--Alberto ## 
    },
#    'WJetsToLNu_HT-600To800_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
#        'nevents' : 3779141,
#        'xsec'    : 12.05,#<--Monojet ## 159.1029/10.,#<-- Julie H ## 12.8,#<--Alberto ##
#        'matcheff': 1.,
#        'kfactor' : 1./0.856705,#<-- Julie H ## 1.21,#<--Flat ## 1.004,#<--Alberto ##
#    },
    'WJetsToLNu_HT-600To800_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 14908339,
        'xsec'    : 12.05,#<--Monojet ## 159.1029/10.,#<-- Julie H ## 12.8,#<--Alberto ##
        'matcheff': 1.,
        'kfactor' : 1.442,#<--Lisav1 ## 1.539,#<--Lisav2 ## 1.21,#<--Flat ## 1./0.856705,#<-- Julie H ## 1.004,#<--Alberto ## 
    },
#    'WJetsToLNu_HT-800To1200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
#        'nevents' : 1544513,
#        'xsec'    : 5.501,#<--Monojet ## 73.4766/10.,#<-- Julie H ## 5.261,#<--Alberto ##
#        'matcheff': 1.,
#        'kfactor' : 1./0.757463,#<-- Julie H ## 1.21,#<--Flat ## 1.004,#<--Alberto ##
#    },
    'WJetsToLNu_HT-800To1200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 6200954,
        'xsec'    : 5.501,#<--Monojet ## 73.4766/10.,#<-- Julie H ## 5.261,#<--Alberto ## 
        'matcheff': 1.,
        'kfactor' : 1.442,#<--Lisav1 ## 1.014,#<--Lisav2 ## 1.21,#<--Flat ## 1./0.757463,#<-- Julie H ## 1.004,#<--Alberto ## 
    },
#    'WJetsToLNu_HT-1200To2500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
#        'nevents' : 244532,
#        'xsec'    : 1.329,#<--Monojet ## 17.5912/10.,#<-- Julie H ## 1.334,#<--Alberto ##
#        'matcheff': 1.,
#        'kfactor' : 1./0.608292,#<-- Julie H ## 1.21,#<--Flat ## 1.004,#<--Alberto ##
#    },
    'WJetsToLNu_HT-1200To2500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 6627909,
        'xsec'    : 1.329,#<--Monojet ## 17.5912/10.,#<-- Julie H ## 1.334,#<--Alberto ## 
        'matcheff': 1.,
        'kfactor' : 1.442,#<--Lisav1 ## 1.014,#<--Lisav2 ## 1.21,#<--Flat ## 1./0.608292,#<-- Julie H ## 1.004,#<--Alberto ## 
    },
#    'WJetsToLNu_HT-2500ToInf_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
#        'nevents' : 253561,
#        'xsec'    : 0.03216,#<--Monojet ## 0.3970/10.,#<-- Julie H ## 0.03089,#<--Alberto ##
#        'matcheff': 1.,
#        'kfactor' : 1./0.454246,#<-- Julie H ## 1.21,#<--Flat ## 1.004,#<--Alberto ##
#    },
    'WJetsToLNu_HT-2500ToInf_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 2384260,
        'xsec'    : 0.03216,#<--Monojet ## 0.3970/10.,#<-- Julie H ## 0.03089,#<--Alberto ## 
        'matcheff': 1.,
        'kfactor' : 1.442,#<--Lisav1 ## 1.014,#<--Lisav2 ## 1.21,#<--Flat ## 1./0.454246,#<-- Julie H ## 1.004,#<--Alberto ## 
    },
    #WJetsToLNu_Pt*
    'WJetsToLNu_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 10089661,
        'xsec'    : 676.3,
        'matcheff': 1.,
        'kfactor' : 1.042,#<--Lisav2 ## 1.038,#<--Lisav1 ##1.,
    },
    'WJetsToLNu_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 10088599,
        'xsec'    : 676.3,
        'matcheff': 1.,
        'kfactor' : 1.042,#<--Lisav2 ## 1.038,#<--Lisav1 ##1.,
    },
    'WJetsToLNu_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 1001250,
        'xsec'    : 23.94,
        'matcheff': 1.,
        'kfactor' : 1.159,#<--Lisav2 ## 1.117,#<--Lisav1 ##1.,
    },
    'WJetsToLNu_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 1000132,
        'xsec'    : 23.94,
        'matcheff': 1.,
        'kfactor' : 1.159,#<--Lisav2 ## 1.117,#<--Lisav1 ##1.,
    },
    'WJetsToLNu_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext4-v1' : {
        'nevents' : 10021205,
        'xsec'    : 23.94,
        'matcheff': 1.,
        'kfactor' : 1.159,#<--Lisav2 ## 1.117,#<--Lisav1 ##1.,
    },
    'WJetsToLNu_Pt-400To600_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 951713,
        'xsec'    : 3.031,
        'matcheff': 1.,
        'kfactor' : 1.299,#<--Lisav2 ## 1.259,#<--Lisav1 ##1.,
    },
    'WJetsToLNu_Pt-400To600_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 988234,
        'xsec'    : 3.031,
        'matcheff': 1.,
        'kfactor' : 1.299,#<--Lisav2 ## 1.259,#<--Lisav1 ##1.,
    },
    'WJetsToLNu_Pt-600ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 989482,
        'xsec'    : 0.4524,
        'matcheff': 1.,
        'kfactor' : 1.124,#<--Lisav2 ## 1.069,#<--Lisav1 ##1.,
    },
    'WJetsToLNu_Pt-600ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 985127,
        'xsec'    : 0.4524 ,
        'matcheff': 1.,
        'kfactor' : 1.124,#<--Lisav2 ## 1.069,#<--Lisav1 ##1.,
    },
#WJetsToLNu NLO inclusive
    'WJetsToLNu_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 24120319,
        'xsec'    : 60290.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },    
##DYJetsToLL_Pt
    'DYJetsToLL_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v2' : {
        'nevents' : 2040596,
        'xsec'    : 83.12 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 2950812,
        'xsec'    : 83.12 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext2-v1' : {
        'nevents' : 2991815,
        'xsec'    : 83.12 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext5-v1' : {
        'nevents' : 75702951,
        'xsec'    : 83.12 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    'DYJetsToLL_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 423976,
        'xsec'    : 3.047 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 590806,
        'xsec'    : 3.047 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext2-v1' : {
        'nevents' : 594317,
        'xsec'    : 3.047 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext5-v1' : {
        'nevents' : 19575946,
        'xsec'    : 3.047 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    'DYJetsToLL_Pt-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 432056,
        'xsec'    : 0.3921 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 589842,
        'xsec'    : 0.3921 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext2-v1' : {
        'nevents' : 604038,
        'xsec'    : 0.3921 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    'DYJetsToLL_Pt-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1' : {
        'nevents' : 430691,
        'xsec'    : 0.03636 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1' : {
        'nevents' : 599665,
        'xsec'    : 0.03636 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'DYJetsToLL_Pt-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext2-v1' : {
        'nevents' : 597526,
        'xsec'    : 0.03636 ,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    
##TT
    'TT_TuneCUETP8M2T4_13TeV-powheg-pythia8-v1' : {
        'nevents' : 77229341,
        'xsec'    : 831.76,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'TTJets_DiLept_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 24350202,
        'xsec'    : 831.76*(1.-0.6760)*(1.-0.6760), # 87.31483776
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'TTJets_SingleLeptFromT_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 50016934, 
        'xsec'    : 831.73*(1-0.6760)*0.6760, # 364.33766304
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'TTJets_SingleLeptFromTbar_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 48266353,
        'xsec'    : 831.73*(1-0.6760)*0.6760, # 364.33766304
        'matcheff': 1.,
        'kfactor' : 1.,
    },
##TTJets backup
#    'TTJets_TuneCUETP8M2T4_13TeV-amcatnloFXFX-pythia8_backup-v1' : {
#        'nevents' : 43561608,
#        'xsec'    : 0.,#NO GENSIM PARENTS
#        'matcheff': 1.,
#        'kfactor' : 1.,
#    },
##ST
    'ST_s-channel_4f_leptonDecays_13TeV-amcatnlo-pythia8_TuneCUETP8M1-v1' : {
        'nevents' : 1000000.,
        'xsec'    : 10.32*(1.-0.6760),
        'kfactor' : 1.,
    },
    'ST_t-channel_antitop_4f_inclusiveDecays_13TeV-powhegV2-madspin-pythia8_TuneCUETP8M1-v1' : {
        'nevents' : 38811017.,
        'xsec'    : 80.95,
        'kfactor' : 1.,
    },
    'ST_t-channel_top_4f_inclusiveDecays_13TeV-powhegV2-madspin-pythia8_TuneCUETP8M1-v1' : {
        'nevents' : 67240808.,
        'xsec'    : 136.02,
        'kfactor' : 1.,
    },
    
    'ST_tW_antitop_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M1_ext1-v1' : {
        'nevents' : 6933094,
        'xsec'    : 71.7/2.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'ST_tW_antitop_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M2T4-v1' : {
        'nevents' : 998276,
        'xsec'    : 71.7/2.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'ST_tW_top_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M1_ext1-v1' : {
        'nevents' : 6952830,
        'xsec'    : 71.7/2.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'ST_tW_top_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M2T4-v1' : {
        'nevents' : 992024,
        'xsec'    : 71.7/2.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    'ST_tW_top_5f_NoFullyHadronicDecays_13TeV-powheg_TuneCUETP8M1-v1' : {
        'nevents' : 5372991.,
        'xsec'    : 71.7/2.*0.557775,
        'kfactor' : 1.,
    },
    'ST_tW_antitop_5f_NoFullyHadronicDecays_13TeV-powheg_TuneCUETP8M1-v1' : {
        'nevents' : 5425134.,
        'xsec'    : 71.7/2.*0.557775,
        'kfactor' : 1.,
    },
    
##WW
    'WW_TuneCUETP8M1_13TeV-pythia8-v1' : {
        'nevents' : 994012,
        'xsec'    : 118.7,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WW_TuneCUETP8M1_13TeV-pythia8_ext1-v1' : {
        'nevents' : 6987124,
        'xsec'    : 118.7,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
##WZ
    'WZ_TuneCUETP8M1_13TeV-pythia8-v1' : {
        'nevents' : 1000000,
        'xsec'    : 47.2,
        'matcheff': 1.,
        'kfactor' : 1.,
    }, 
    'WZTo2L2Q_13TeV_amcatnloFXFX_madspin_pythia8-v1' : {
        'nevents' : 998034,
        'xsec'    : 5.595,
        'matcheff': 1.,
        'kfactor' : 1.,
    },    
    'WZ_TuneCUETP8M1_13TeV-pythia8_ext1-v1' : {
        'nevents' : 2995828,
        'xsec'    : 47.2,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
#ZZ
    'ZZ_TuneCUETP8M1_13TeV-pythia8-v1' : {
        'nevents' : 990064,
        'xsec'    : 16.6,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'ZZ_TuneCUETP8M1_13TeV-pythia8_ext1-v1' : {
        'nevents' : 998034,
        'xsec'    : 16.6,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'ZZTo2L2Q_13TeV_amcatnloFXFX_madspin_pythia8-v1' : {
        'nevents' : 15345572,
        'xsec'    : 3.22,
        'matcheff': 1.,
        'kfactor' : 1.,
    },   
    
    # FIXME - check xs for QCD (taken from ichep)
    'QCD_HT100to200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
        'nevents' : 80684349,
        'xsec'    : 27990000.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT200to300_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
        'nevents' : 18722416,
        'xsec'    : 1712000.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT200to300_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 38857977,
        'xsec'    : 1712000.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT300to500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
        'nevents' : 17035891,
        'xsec'    : 347700.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT300to500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 37502012,
        'xsec'    : 347700.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT500to700_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
        'nevents' : 18929951,
        'xsec'    : 32100.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT500to700_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v2' : {
        'nevents' : 43341392,
        'xsec'    : 32100.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT700to1000_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
        'nevents' : 15629253,
        'xsec'    : 6831.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT700to1000_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 29783527,
        'xsec'    : 6831.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },   
    'QCD_HT1000to1500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
        'nevents' : 4767100,
        'xsec'    : 1207.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT1000to1500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 10360193,
        'xsec'    : 1207.,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT1500to2000_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
        'nevents' : 3970819,
        'xsec'    : 119.9,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT1500to2000_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 7855883,
        'xsec'    : 119.9,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT2000toInf_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1' : {
        'nevents' : 1991645,
        'xsec'    : 25.24,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'QCD_HT2000toInf_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1' : {
        'nevents' : 4047360,
        'xsec'    : 25.24,
        'matcheff': 1.,
        'kfactor' : 1.,
    },


    ### Signal ###
    #ZHadZinv
    'BulkGravToZZToZhadZinv_narrow_M-600_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-800_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-1000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-1200_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-1400_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-1600_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-1800_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-2000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-2500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-3000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-3500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-4000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZhadZinv_narrow_M-4500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.6991*0.20,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    #WHadZInv
    'WprimeToWZToWhadZinv_narrow_M-600_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-800_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-1000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-1200_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-1400_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-1600_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-1800_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-2000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-2500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-3000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-3500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-4000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZinv_narrow_M-4500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.20*0.6741,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    #WHadZLep
    'WprimeToWZToWhadZlep_narrow_M-600_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-800_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-1000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-1200_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-1400_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-1600_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-1800_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-2000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-2500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-3000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-3500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-4000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'WprimeToWZToWhadZlep_narrow_M-4500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 1.*0.033658*3*0.6760,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    #ZHadZLep
    'BulkGravToZZToZlepZhad_narrow_M-600_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-650_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-700_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-750_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-800_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-1000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-1200_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-1400_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-1800_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-2000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-2500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-3000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-3500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-4000_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'BulkGravToZZToZlepZhad_narrow_M-4500_13TeV-madgraph-v1' : {
        'nevents' : 100000,
        'xsec'    : 2.*0.033658*3*0.6991,
        'matcheff': 1.,
        'kfactor' : 1.,
    },

    #HH FIXME - bsm xs
    'GluGluToHHTo4B_node_2_13TeV-madgraph-v1' : {
        'nevents' : 299600,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_3_13TeV-madgraph-v1' : {
        'nevents' : 299800,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_4_13TeV-madgraph-v1' : {
        'nevents' : 297000,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_5_13TeV-madgraph-v1' : {
        'nevents' : 300000,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_6_13TeV-madgraph-v1' : {
        'nevents' : 300000,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_7_13TeV-madgraph-v1' : {
        'nevents' : 300000,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_8_13TeV-madgraph-v1' : {
        'nevents' : 300000,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_9_13TeV-madgraph-v1' : {
        'nevents' : 298200,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_10_13TeV-madgraph-v1' : {
        'nevents' : 300000,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_11_13TeV-madgraph-v1' : {
        'nevents' : 300000,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_12_13TeV-madgraph-v1' : {
        'nevents' : 299400,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_13_13TeV-madgraph-v1' : {
        'nevents' : 300000,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_box_13TeV-madgraph-v1' : {
        'nevents' : 278600,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'GluGluToHHTo4B_node_SM_13TeV-madgraph-v1' : {
        'nevents' : 299800,
        'xsec'    : 0.03345*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    'VBFHHTo4B_CV_1_C2V_1_C3_1_13TeV-madgraph-v1' : {
        'nevents' : 297969,
        'xsec'    : 1.*0.577*0.577,
        'matcheff': 1.,
        'kfactor' : 1.,
    },
    
}





samples = {
    'data_obs' : {
        'order' : 0,
        'files' : ['METRun2016B-03Feb2017_ver1-v1', 'METRun2016B-03Feb2017_ver2-v2', 'METRun2016C-03Feb2017-v1', 'METRun2016D-03Feb2017-v1', 'METRun2016E-03Feb2017-v1', 'METRun2016F-03Feb2017-v1', 'METRun2016G-03Feb2017-v1', 'METRun2016H-03Feb2017_ver2-v1', 'METRun2016H-03Feb2017_ver3-v1'],
 #        'files' : ['METRun2016B-23Sep2016-v2', 'METRun2016B-23Sep2016-v3', 'METRun2016C-23Sep2016-v1', 'METRun2016D-23Sep2016-v1', 'METRun2016E-23Sep2016-v1', 'METRun2016F-23Sep2016-v1', 'METRun2016G-23Sep2016-v1', 'METRun2016H-PromptReco-v1', 'METRun2016H-PromptReco-v2', 'METRun2016H-PromptReco-v3'],
        'fillcolor' : 0,
        'fillstyle' : 1,
        'linecolor' : 1,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "Data",
        'weight': 1.,
        'plot': True,
    },
    'data_obs1' : {
        'order' : 0,
        'files' : ['METRun2016B-PromptReco-v1', 'METRun2016B-PromptReco-v2', 'METRun2016C-PromptReco-v2', 'METRun2016D-PromptReco-v2', 'METRun2016E-PromptReco-v2', 'METRun2016F-PromptReco-v1', 'METRun2016G-PromptReco-v1', 'METRun2016H-PromptReco-v1', 'METRun2016H-PromptReco-v2', 'METRun2016H-PromptReco-v3'],
 #        'files' : ['METRun2016B-23Sep2016-v2', 'METRun2016B-23Sep2016-v3', 'METRun2016C-23Sep2016-v1', 'METRun2016D-23Sep2016-v1', 'METRun2016E-23Sep2016-v1', 'METRun2016F-23Sep2016-v1', 'METRun2016G-23Sep2016-v1', 'METRun2016H-PromptReco-v1', 'METRun2016H-PromptReco-v2', 'METRun2016H-PromptReco-v3'],
        'fillcolor' : 0,
        'fillstyle' : 1,
        'linecolor' : 1,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "Data",
        'weight': 1.,
        'plot': True,
    },

    'DYJetsToLL_Pt' : {
        'order' : 1,
        'files' : ['DYJetsToLL_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1',
                   'DYJetsToLL_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1',
                    'DYJetsToLL_Pt-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1',
                    'DYJetsToLL_Pt-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1',],
        'fillcolor' : 418,
        'fillstyle' : 1001,
        'linecolor' : 418,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "Z(ll) + jets",
        'weight': 1.,
        'plot': True,
    },

    'DYJetsToNuNu_PtZ' : {
        'order' : 1,
        'files' : ['DYJetsToNuNu_PtZ-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','DYJetsToNuNu_PtZ-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1','DYJetsToNuNu_PtZ-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','DYJetsToNuNu_PtZ-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1'],#['DYJetsToNuNu_PtZ-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','DYJetsToNuNu_PtZ-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1','DYJetsToNuNu_PtZ-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1','DYJetsToNuNu_PtZ-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','DYJetsToNuNu_PtZ-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1','DYJetsToNuNu_PtZ-50To100_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','DYJetsToNuNu_PtZ-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','DYJetsToNuNu_PtZ-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1'],
        'fillcolor' : 856,
        'fillstyle' : 1001,
        'linecolor' : 856,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "Z(#nu#nu) + jets Pt",
        'weight': 1.,
        'plot': True,
    },

    'DYJetsToNuNu_PtZ1' : {
        'order' : 1,
        'files' : ['DYJetsToNuNu_PtZ-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1', 'DYJetsToNuNu_PtZ-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1', 'DYJetsToNuNu_PtZ-400To650_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1', 'DYJetsToNuNu_PtZ-650ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1'],
        'fillcolor' : 856,
        'fillstyle' : 1001,
        'linecolor' : 856,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "Z(#nu#nu) + jets",
        'weight': 1.,
        'plot': True,
    },

    'ZJetsToNuNu_HT' : {
        'order' : 1,
        'files' : ['ZJetsToNuNu_HT-100To200_13TeV-madgraph-v1', 'ZJetsToNuNu_HT-200To400_13TeV-madgraph-v1', 'ZJetsToNuNu_HT-400To600_13TeV-madgraph-v1', 'ZJetsToNuNu_HT-600To800_13TeV-madgraph-v1', 'ZJetsToNuNu_HT-800To1200_13TeV-madgraph-v1', 'ZJetsToNuNu_HT-1200To2500_13TeV-madgraph-v1', 'ZJetsToNuNu_HT-2500ToInf_13TeV-madgraph-v1'],
        'fillcolor' : 856,
        'fillstyle' : 1001,
        'linecolor' : 856,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "Z(#nu#nu) + jets HT",
        'weight': 1.,
        'plot': True,
    },
    
    'WJetsToLNu_Incl' : {
        'order' : 1,
        'files' : ['WJetsToLNu_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1'],
        'fillcolor' : 881,
        'fillstyle' : 1001,
        'linecolor' : 881,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "W(l#nu) + jets Incl",
        'weight': 1.,
        'plot': True,
    },


    'WJetsToLNu_Pt' : {
        'order' : 1,
        'files' : ['WJetsToLNu_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','WJetsToLNu_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','WJetsToLNu_Pt-400To600_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','WJetsToLNu_Pt-600ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1'],#['WJetsToLNu_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','WJetsToLNu_Pt-100To250_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1','WJetsToLNu_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','WJetsToLNu_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1','WJetsToLNu_Pt-250To400_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext4-v1','WJetsToLNu_Pt-400To600_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','WJetsToLNu_Pt-400To600_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1','WJetsToLNu_Pt-600ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8-v1','WJetsToLNu_Pt-600ToInf_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8_ext1-v1'],
        'fillcolor' : 881,
        'fillstyle' : 1001,
        'linecolor' : 881,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "W(l#nu) + jets Pt",
        'weight': 1.,
        'plot': True,
    },

    'WJetsToLNu_HT' : {
        'order' : 1,
        'files' : ['WJetsToLNu_HT-100To200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1','WJetsToLNu_HT-200To400_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1','WJetsToLNu_HT-400To600_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1','WJetsToLNu_HT-600To800_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1','WJetsToLNu_HT-800To1200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1','WJetsToLNu_HT-1200To2500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1','WJetsToLNu_HT-2500ToInf_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1'],
        'fillcolor' : 881,
        'fillstyle' : 1001,
        'linecolor' : 881,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "W(l#nu) + jets HT",
        'weight': 1.,
        'plot': True,
    },

    'WJetsToLNu_HT1' : {
        'order' : 1,
        'files' : ['WJetsToLNu_HT-100To200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1', 'WJetsToLNu_HT-200To400_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1', 'WJetsToLNu_HT-400To600_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1', 'WJetsToLNu_HT-600To800_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1', 'WJetsToLNu_HT-800To1200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v2', 'WJetsToLNu_HT-1200To2500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_ext1-v1', 'WJetsToLNu_HT-2500ToInf_TuneCUETP8M1_13TeV-madgraphMLM-pythia8-v1'],
        'fillcolor' : 881,
        'fillstyle' : 1001,
        'linecolor' : 881,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "W(l#nu) + jets",
        'weight': 1.,
        'plot': True,
    },

    'TTbar' : {
        'order' : 3,
        'files' : ['TT_TuneCUETP8M2T4_13TeV-powheg-pythia8-v1',],
        'fillcolor' : 798,
        'fillstyle' : 1001,
        'linecolor' : 798,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "t#bar{t}",
        'weight': 1.,
        'plot': True,
    },

    'TTbar1' : {
        'order' : 3,
        'files' : ['TT_TuneCUETP8M1_13TeV-powheg-pythia8_ext4-v1',],
        'fillcolor' : 798,
        'fillstyle' : 1001,
        'linecolor' : 798,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "t#bar{t}",
        'weight': 1.,
        'plot': True,
    },

    'ST' : {
        'order' : 4,
        'files' : ['ST_tW_antitop_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M1_ext1-v1','ST_tW_top_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M1_ext1-v1','ST_s-channel_4f_leptonDecays_13TeV-amcatnlo-pythia8_TuneCUETP8M1-v1','ST_t-channel_antitop_4f_inclusiveDecays_13TeV-powhegV2-madspin-pythia8_TuneCUETP8M1-v1','ST_t-channel_top_4f_inclusiveDecays_13TeV-powhegV2-madspin-pythia8_TuneCUETP8M1-v1'],#['ST_tW_antitop_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M2T4-v1','ST_tW_top_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M2T4-v1'],
        'fillcolor' : 801,
        'fillstyle' : 1001,
        'linecolor' : 801,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "Single-t",
        'weight': 1.,
        'plot': True,
    },
    'ST1' : {
        'order' : 4,
        'files' : ['ST_tW_antitop_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M1-v1','ST_tW_top_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M1-v2',],
        'fillcolor' : 801,
        'fillstyle' : 1001,
        'linecolor' : 801,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "Single-t",
        'weight': 1.,
        'plot': True,
    },

    'Top' : {
        'order' : 4,
        'files' : ['TT_TuneCUETP8M2T4_13TeV-powheg-pythia8-v1','ST_tW_antitop_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M1_ext1-v1','ST_tW_top_5f_inclusiveDecays_13TeV-powheg-pythia8_TuneCUETP8M1_ext1-v1','ST_s-channel_4f_leptonDecays_13TeV-amcatnlo-pythia8_TuneCUETP8M1-v1','ST_t-channel_antitop_4f_inclusiveDecays_13TeV-powhegV2-madspin-pythia8_TuneCUETP8M1-v1','ST_t-channel_top_4f_inclusiveDecays_13TeV-powhegV2-madspin-pythia8_TuneCUETP8M1-v1'],
        'fillcolor' : 801,
        'fillstyle' : 1001,
        'linecolor' : 801,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "Single-t",
        'weight': 1.,
        'plot': True,
    },

    'VV' : {
        'order' : 5,
        'files' : ['WW_TuneCUETP8M1_13TeV-pythia8-v1','WZ_TuneCUETP8M1_13TeV-pythia8-v1','ZZ_TuneCUETP8M1_13TeV-pythia8-v1'],#'ZZ_TuneCUETP8M1_13TeV-pythia8_ext1-v1',
        'fillcolor' : 602,
        'fillstyle' : 1001,
        'linecolor' : 602,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "VV",
        'weight': 1.,
        'plot': True,
    },

    'VV1' : {
        'order' : 5,
        'files' : ['WW_TuneCUETP8M1_13TeV-pythia8-v1','WZ_TuneCUETP8M1_13TeV-pythia8-v1','ZZ_TuneCUETP8M1_13TeV-pythia8-v1'],#'ZZ_TuneCUETP8M1_13TeV-pythia8_ext1-v1',
        'fillcolor' : 602,
        'fillstyle' : 1001,
        'linecolor' : 602,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "VV",
        'weight': 1.,
        'plot': True,
    },
    
    'VV2L2Q' : {
        'order' : 5,
        'files' : ['ZZTo2L2Q_13TeV_amcatnloFXFX_madspin_pythia8-v1', 'WZTo2L2Q_13TeV_amcatnloFXFX_madspin_pythia8-v1'],
        'fillcolor' : 602,
        'fillstyle' : 1001,
        'linecolor' : 602,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "VV",
        'weight': 1.,
        'plot': True,
    },
    
    # Dummy entry for background sum
    'BkgSum' : {
        'order' : 0,
        'files' : [],
        'fillcolor' : 1,
        'fillstyle' : 3003,
        'linecolor' : 1,
        'linewidth' : 2,
        'linestyle' : 1,
        'label' : "MC stat.",
        'weight': 1.,
        'plot': True,
    },
    
    ### BulkG -> Zhad Zinv
    'XZZInv_M600' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-600_13TeV-madgraph-v1'],
        'fillcolor' : 826,#51,
        'fillstyle' : 3344,#3005,
        'linecolor' : 826,#51,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 600 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M800' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-800_13TeV-madgraph-v1'],
        'fillcolor' : 827,#52,
        'fillstyle' : 3344,#3005,
        'linecolor' : 827,#52,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 800 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M1000' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-1000_13TeV-madgraph-v1'],
        'fillcolor' : 829,#53,
        'fillstyle' : 3344,#3005,
        'linecolor' : 829,#53,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 1000 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M1200' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-1200_13TeV-madgraph-v1'],
        'fillcolor' : 840,#55,
        'fillstyle' : 3344,#3005,
        'linecolor' : 840,#55,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 1200 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M1400' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-1400_13TeV-madgraph-v1'],
        'fillcolor' : 831,#57,
        'fillstyle' : 3344,#3005,
        'linecolor' : 831,#57,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 1400 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M1600' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-1600_13TeV-madgraph-v1'],
        'fillcolor' : 832,#59,
        'fillstyle' : 3344,#3005,
        'linecolor' : 832,#59,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 1600 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M1800' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-1800_13TeV-madgraph-v1'],
        'fillcolor' : 833,#61,
        'fillstyle' : 3344,#3005,
        'linecolor' : 833,#61,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 1800 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M2000' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-2000_13TeV-madgraph-v1'],
        'fillcolor' : 834,#63,
        'fillstyle' : 3344,#3005,
        'linecolor' : 834,#63,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 2000 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M2500' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-2500_13TeV-madgraph-v1'],
        'fillcolor' : 843,#65,
        'fillstyle' : 3344,#3005,
        'linecolor' : 843,#65,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 2500 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M3000' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-3000_13TeV-madgraph-v1'],
        'fillcolor' : 845,#67,
        'fillstyle' : 3344,#3005,
        'linecolor' : 845,#67,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 3000 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M3500' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-3500_13TeV-madgraph-v1'],
        'fillcolor' : 847,#69,
        'fillstyle' : 3344,#3005,
        'linecolor' : 847,#69,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 3500 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M4000' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-4000_13TeV-madgraph-v1'],
        'fillcolor' : 849,#71,
        'fillstyle' : 3344,#3005,
        'linecolor' : 849,#71,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 4000 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XZZInv_M4500' : {
        'order' : 1001,
        'files' : ['BulkGravToZZToZhadZinv_narrow_M-4500_13TeV-madgraph-v1'],
        'fillcolor' : 850,#73,
        'fillstyle' : 3344,#3005,
        'linecolor' : 850,#73,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{G} = 4500 GeV",
        'weight': 1.,
        'plot': True,
    },

    ### BulkG -> Zhad Zinv
    'XWZInv_M600' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-600_13TeV-madgraph-v1'],
        'fillcolor' : 826,#51,
        'fillstyle' : 3344,#3005,
        'linecolor' : 826,#51,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 600 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M800' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-800_13TeV-madgraph-v1'],
        'fillcolor' : 827,#52,
        'fillstyle' : 3344,#3005,
        'linecolor' : 827,#52,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 800 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M1000' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-1000_13TeV-madgraph-v1'],
        'fillcolor' : 829,#53,
        'fillstyle' : 3344,#3005,
        'linecolor' : 829,#53,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 1000 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M1200' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-1200_13TeV-madgraph-v1'],
        'fillcolor' : 840,#55,
        'fillstyle' : 3344,#3005,
        'linecolor' : 840,#55,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 1200 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M1400' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-1400_13TeV-madgraph-v1'],
        'fillcolor' : 831,#57,
        'fillstyle' : 3344,#3005,
        'linecolor' : 831,#57,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 1400 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M1600' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-1600_13TeV-madgraph-v1'],
        'fillcolor' : 832,#59,
        'fillstyle' : 3344,#3005,
        'linecolor' : 832,#59,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 1600 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M1800' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-1800_13TeV-madgraph-v1'],
        'fillcolor' : 833,#61,
        'fillstyle' : 3344,#3005,
        'linecolor' : 833,#61,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 1800 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M2000' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-2000_13TeV-madgraph-v1'],
        'fillcolor' : 834,#841,
        'fillstyle' : 3344,#3005,
        'linecolor' : 834,#841#63,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 2000 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M2500' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-2500_13TeV-madgraph-v1'],
        'fillcolor' : 843,#65,
        'fillstyle' : 3344,#3005,
        'linecolor' : 843,#65,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 2500 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M3000' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-3000_13TeV-madgraph-v1'],
        'fillcolor' : 845,#67,
        'fillstyle' : 3344,#3005,
        'linecolor' : 845,#67,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 3000 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M3500' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-3500_13TeV-madgraph-v1'],
        'fillcolor' : 847,#69,
        'fillstyle' : 3344,#3005,
        'linecolor' : 847,#69,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 3500 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M4000' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-4000_13TeV-madgraph-v1'],
        'fillcolor' : 849,#71,
        'fillstyle' : 3344,#3005,
        'linecolor' : 849,#71,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 4000 GeV",
        'weight': 1.,
        'plot': True,
    },
    'XWZInv_M4500' : {
        'order' : 1001,
        'files' : ['WprimeToWZToWhadZinv_narrow_M-4500_13TeV-madgraph-v1'],
        'fillcolor' : 850,#73,
        'fillstyle' : 3344,#3005,
        'linecolor' : 850,#73,
        'linewidth' : 3,
        'linestyle' : 1,
        'label' : "m_{W^{'}} = 4500 GeV",
        'weight': 1.,
        'plot': True,
    },
    
}
