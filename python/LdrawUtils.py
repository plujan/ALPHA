...
def project(var, cut, weight, samplelist, pd, ntupledir, treename="ntuple/tree"):
    # Create dict
    file = {}
    tree = {}
    chain = {}
...
def draw(hist, data, back, sign, snorm=1, ratio=0, poisson=False, log=False):
    ...
    # --- Display ---
    c1 = TCanvas("c1", hist.values()[-1].GetXaxis().GetTitle(), 1000, 800 if ratio else 600)
...
def drawAnalysis(s, center=False):
    analyses = {"VZ" : "X #rightarrow ZV #rightarrow llqq", "VZinv" : "X #rightarrow VZ #rightarrow qq#nu#nu", "VH" : "X #rightarrow Vh #rightarrow (ll,l#nu,#nu#nu)bb", "Vh" : "X #rightarrow Vh #rightarrow (ll,l#nu,#nu#nu)bb", "Zh" : "Z' #rightarrow Zh #rightarrow (ll,#nu#nu)bb", "Wh" : "W' #rightarrow Wh #rightarrow l#nu bb", "DM": "DM + heavy flavour", "AZh" : "A #rightarrow Zh #rightarrow llbb", "ZZ" : "G #rightarrow ZZ #rightarrow llqq", "WZ" : "W' #rightarrow ZW #rightarrow llqq", }
    latex = TLatex()
    latex.SetNDC()
    latex.SetTextSize(0.04)
    latex.SetTextFont(42)
